...
public class MainActivity extends Activity implements OnClickListener {
    ...
    //onClick function activates when a button is hit.

    @Override
    public void onClick(View v) {

        // The button that was pressed is stored in this variable
        String buttonPressed = ((Button) v).getText().toString();

        // DIGITS is a string containing "0123456789." this IF will help differentiate between a number and an operator
        if (DIGITS.contains(buttonPressed)) {

            // For the first digit pressed,
            if (userIsInTheMiddleOfTypingANumber) {

                // If the button pressed is "." and the current display contains a "." then don't append
